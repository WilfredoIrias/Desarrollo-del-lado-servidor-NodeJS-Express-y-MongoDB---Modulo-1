...
Bicicleta.create('Rojo', 'Urbana', [41.3879, 2.1699]);
Bicicleta.create('Azul', 'Montana', [41.3825, 2.1769]);
Bicicleta.create('Verde', 'Plegable', [41.3851, 2.1734]);
Bicicleta.create('Negro', 'Electrica', [41.3888, 2.1580]);
...
app.get('/', (req, res) => {
    ...
    <body>
        <div class="container">
            <h1>🚲 BIKEAPI</h1>
            <h2>¡Bienvenido a Express!</h2>
            <p>Servidor funcionando correctamente</p>
            <p>Version mejorada con mapas interactivos</p>
            <div class="links">
...
app.get('/mapa', (req, res) => {
    const bicicletas = Bicicleta.findAll();
    ...
    <body>
        ...
        <script>
            ...
            bicicletas.forEach(bici => {
                ...
                marker.bindPopup(\`
                    <b>🚲 Bicicleta #\${bici.id}</b><br>
                    Color: \${bici.color}<br>
                    Modelo: \${bici.modelo}<br>
                    <small>ID: \${bici.id}</small>
                \`);
            });
...
app.get('/api/bicicletas', (req, res) => {
    res.json({
        success: true,
        data: Bicicleta.findAll(),
        total: Bicicleta.findAll().length
    });
});

app.get('/api/bicicletas/:id', (req, res) => {
    const bici = Bicicleta.findById(parseInt(req.params.id));
    if (bici) {
...
app.post('/api/bicicletas', (req, res) => {
    ...
    const nuevaBici = Bicicleta.create(color, modelo, [parseFloat(lat), parseFloat(lng)]);
    res.status(201).json({
        success: true,
        data: nuevaBici,
...
app.put('/api/bicicletas/:id', (req, res) => {
    ...
    const biciActualizada = Bicicleta.update(id, datosActualizados);
...
app.delete('/api/bicicletas/:id', (req, res) => {
    const eliminado = Bicicleta.delete(parseInt(req.params.id));
